...
SEARCH_INFO bfs(Node root)
{
    ...
    q.push(root);
    distance[state_to_string(root.get_state())] = root.get_g_value();
    ...
    while(!q.empty())
    {
        ...
        for(int i = 0; i < current_node.get_number_of_succs(); i++)
        {
            ...
            if(!distance.count(state_to_string(neighbor.get_state())))
            {
                distance[state_to_string(neighbor.get_state())] = neighbor.get_g_value();
                q.push(neighbor);
            }
        }
    }
...
{
    std::priority_queue<Node_15, std::vector<Node_15>, compare_nodes_a_star_15> q;
    std::unordered_map<std::string, float> distance;
